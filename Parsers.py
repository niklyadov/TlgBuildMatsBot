...
def search1(searchword):
    html = requests.get('https://radugastroy.ru/search/?module=search&action=search&searchword=' + searchword).text
...
def search2(searchword):
    html = requests.get('https://www.sdvor.com/moscow/search/?only_in_stock=true&str=' + searchword).text
    soup = BeautifulSoup(html)
...
def search3(searchword):
    html = requests.get('https://s-b-1.ru/catalog/?q=' + searchword).text
    soup = BeautifulSoup(html)
